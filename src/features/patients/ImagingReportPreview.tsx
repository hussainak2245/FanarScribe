...
export function ImagingReportPreview({ report }: { report: ReportPreview }) {
  return (
    <SoftPanel className="p-4" tone="mist">
      <div className="mb-2 flex items-center justify-between gap-3">
        <p className="flex items-center gap-2 text-sm font-black text-navy-900">
          <ScanSearch className="h-4 w-4 text-cyan-500" aria-hidden="true" />
          {report.title}
        </p>
        <Badge tone="cyan">{formatDate(report.date)}</Badge>
      </div>
